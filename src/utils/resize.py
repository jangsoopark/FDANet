...
def imresize(image, scalar_scale=None, method='bicubic', output_shape=None, mode="vec"):
    if method == 'bicubic':
        kernel = cubic
    ...
    if scalar_scale is not None:
        ...
        scale = derive_scale_from_size(image.shape, output_shape)
    ...
    indices = []
    for k in range(2):
        w, ind = contributions(image.shape[k], output_size[k], scale[k], kernel, kernel_width)
        weights.append(w)
        indices.append(ind)
    b = np.copy(image)
    flag_2d = False
