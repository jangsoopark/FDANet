...
parser = argparse.ArgumentParser(description='WHU Change Data Patch Extraction')
parser.add_argument('--dataset-root', type=str,
                    default='/workspace/dataset/WHU-Dataset/',
                    help='Original dataset root')
parser.add_argument('--experiment-root', type=str,
                    default='/workspace/dataset/WHU-experiment/',
...
def augmentation(x1_path, x2_path, y_path, patch_size, stride, rotates, flips, scales, dst_root):
    x1 = rasterio.open(x1_path).read()
    x2 = rasterio.open(x2_path).read()
    y = rasterio.open(y_path).read()
    
    x1 = np.transpose(x1, axes=(1, 2, 0))
    x2 = np.transpose(x2, axes=(1, 2, 0))
    y = np.transpose(y, axes=(1, 2, 0)) * 255
...
def main():
    ...
    _image_a_pattern = os.path.join(args.dataset_root, f'1.image-dataset/2012/whole_image/{args.mode}/image/*.tif')
    _image_b_pattern = os.path.join(args.dataset_root, f'1.image-dataset/2016/whole_image/{args.mode}/image/*.tif')
    _label_pattern = os.path.join(args.dataset_root, f'1.image-dataset/change_label/{args.mode}/*.tif')
    ...
    if args.mode == 'test':
        shutil.copytree(dst_root, dst_root.replace('test', 'val'))
